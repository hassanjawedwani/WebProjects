      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 '>
        {
          products.filter((product) =>product.inStock).slice(0, 6).map((product, index) => (
            <div key={index} className='group rounded-lg flex flex-col justify-center  p-4 border border-gray-200 gap-1'>
              ...
              <div className=' flex flex-row justify-between gap-2 mt-2'>  {/*onClick={e => e.stopPropagation()}*/}
                ...
                {cartItems[product._id] 
                  ...
                  :
                  (
                    <button className='text-sm text-primary border border-primary py-1 px-2 flex gap-2 shrink-0 w-20 justify-center' onClick={() => { addToCart(product._id); setCount(prevCount => Math.max(prevCount + 1, 0)); }}>
